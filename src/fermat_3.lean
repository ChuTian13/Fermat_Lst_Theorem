...
lemma prop_3_1 (h: is_primitive_root ζ 3) : ζ^3 = 1
:= by exact h.pow_eq_one 
lemma prop_3_2 (h: is_primitive_root ζ 3) : 1 + ζ + ζ^2 = 0
:= 
begin
  have h1: (ζ - 1)*(1 + ζ + ζ^2) = (ζ - 1) * 0,
  {calc (ζ - 1)*(1 + ζ + ζ^2) = ζ^3 - 1     : by ring
                       ...    = 1 - 1       : by rw prop_3_1 ζ h
                       ...    = (ζ - 1) * 0 : by ring,
  },
  have h2: ζ - 1 ≠ 0, 
  {have h3: ζ ≠ 1, 
  {calc ζ = ζ^1  : by ring 
        ... ≠ 1 : by exact is_primitive_root.pow_ne_one_of_pos_of_lt h zero_lt_one h1_lt_3,},
  exact sub_ne_zero.mpr h3},
  exact mul_left_cancel' h2 h1
end

lemma fac_3{x y :ℤ}(h : is_primitive_root ζ 3) : 
...
theorem fac_3_1{x y :ℤ}(h : is_primitive_root ζ 3): 
(coe x)^3 + (coe y)^3 = ((coe x) + (coe y)) * ((coe x) + ζ * (coe y)) * ((coe x) + ζ^2 * (coe y))
:= 
begin
  exact eq.symm (fac_3 ζ h),
end
...
theorem fermat_last_theorem_3 :
 ∀ x y z: ℤ, ∀ n : ℕ, x * y * z ≠ 0 ∧ is_primitive_root ζ 3 → (coe x)^3 + (coe y)^3 ≠ (coe z : ℂ)^3 
:= 
begin
  intros x y z n h, 
  calc (coe x)^3 + (coe y)^3 = ((coe x) + (coe y)) * ((coe x) + ζ * (coe y)) * ((coe x) + ζ^2 * (coe y))
                        : by exact eq.symm(fac_3 ζ h.2)
       ...  ≠ (coe z)^3 : by exact fermat_last_theorem_3' ζ x y z n h.1,
end
